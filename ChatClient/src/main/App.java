...
//        loadAcctGUI();
    ...
    private void loadGUI() {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                joinChat(Constant.SERVER_IP, Constant.PORT);
            }
        });
    }
    ...
    @Override
    public void printMineToScreen(IMessage msg, int port) {
        if (msg != null) {
            if (chatManager.belongsToActiveChat(port)) {
                gui.displayMessage(msg, true);
                db.saveMessage(chatManager.getActiveChat(), msg, true);
            } else {
                db.saveMessage(chatManager.getChatByPort(port), msg, true);
            }
        }
    }

    @Override
    public void showPopup(String text) throws IOException, InterruptedException {
        String me = who.getUsername();
        gui.popup(text, me);
    }
